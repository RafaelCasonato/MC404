...
void pack(int numero, int start_bit, int end_bit, int *val) {
    int size = (end_bit - start_bit) + 1;
    int mask = (potencia(2, size)) - 1;
    mask = (numero & mask) << start_bit;
    *val |= mask;
}
...
int main()
{
    int resultado = 0;
    char input[40];
    int n = read(0, input, 40);
    InstData *data;
    get_inst_data(input, data);
    resultado |= data->opcode;
    switch(data->type) {
        case (R):
            pack(data->rd, 7, 11, &resultado);
            pack(data->funct3, 12, 14, &resultado);
            pack(data->rs1, 15, 19, &resultado);
            pack(data->rs2, 20, 24, &resultado);
            pack(data->funct7, 25, 31, &resultado);
            break;
        case (I):
            pack(data->rd, 7, 11, &resultado);
            pack(data->funct3, 12, 14, &resultado);
            pack(data->rs1, 15, 19, &resultado);
            pack((data->imm), 20, 31, &resultado);
            break;
        case (S):
            pack((data->imm), 7, 11, &resultado);
            pack(data->funct3, 12, 14, &resultado);
            pack(data->rs1, 15, 19, &resultado);
            pack(data->rs2, 20, 24, &resultado);
            pack((data->imm >> 5), 25, 31, &resultado);
            break;
        case (B):
            pack((data->imm >> 11), 7, 7, &resultado);
            pack((data->imm >> 1), 8, 11, &resultado);
            pack(data->funct3, 12, 14, &resultado);
            pack(data->rs1, 15, 19, &resultado);
            pack(data->rs2, 20, 24, &resultado);
            pack((data->imm >> 5), 25, 30, &resultado);
            pack((data->imm >> 12), 31, 31, &resultado);
            break;
        case (U):
            pack(data->rd, 7, 11, &resultado);
            pack((data->imm), 12, 31, &resultado);
            break;
        case (J):
            pack(data->rd, 7, 11, &resultado);
            pack((data->imm >> 12), 12, 19, &resultado);
            pack((data->imm >> 11), 20, 20, &resultado);
            pack((data->imm >> 1), 21, 30, &resultado);
            pack((data->imm >> 20), 31, 31, &resultado);
            break;
    }
    hex_code(resultado);
    return 0;
